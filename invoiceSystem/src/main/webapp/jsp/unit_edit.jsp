	...
	<form action=""  class="form form-horizontal" id="form-member-add">
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<%=request.getParameter("name") %>" placeholder="" id="name" name="rname">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>单位地址：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<%=request.getParameter("addr") %>" placeholder="" id="addr" name="addr">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>增值税号：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<%=request.getParameter("tax_num") %>" placeholder="" id="tax_num" name="tax_num">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>联系电话：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<%=request.getParameter("link_tel") %>" placeholder="" id="link_tel" name="link_tel">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>银行名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<%=request.getParameter("bank_name") %>" placeholder="" id="bank_name" name="bank_name">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>银行帐号：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<%=request.getParameter("bank_num") %>" placeholder="" id="bank_num" name="bank_num">
			</div>
		</div>		
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
				<input class="btn btn-primary radius" onclick="unit_edit('<%=request.getParameter("b_id") %>')" type="button" value="&nbsp;&nbsp;提交&nbsp;&nbsp;">
			</div>
		</div>
	</form>
</article>
...
function unit_edit(id){
	...
	var data={b_id:id,ctm_name:name,ctm_addr:addr,tax_num:taxnum,link_tel:linktel,bank_name:bankname,bank_num:banknum};
    var result = JSON.stringify(data);
		  $.ajax({
				type: "POST",
				url: "http://localhost:8080/unit/update",
				data: result,
				dataType : "json",
				contentType: 'application/json',
				success: function(data){
					if(data.code=="0"){
						layer.msg("已保存!", {icon: 1,time:2000});
